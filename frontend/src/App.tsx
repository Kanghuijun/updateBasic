...
const PostTestPage: React.FC = () => {
  ...
  // 3.5 내용으로 게시물 검색 추가 (GET /post/search?content= ...)
  const handleContentSerach = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!contentSearchTerm) {
      setMessage('검색할 내용을 입력하세요.');
      return;
    }
    setLoading(true);
    setMessage(`'${contentSearchTerm}' 내용으로 검색 중입니다...`);
    try {
      const response = await fetch(`${API_BASE_URL}/search/content?content=${encodeURIComponent(contentSearchTerm)}`);
      const result = await response.json();
      if (!response.ok) throw new Error(result.message || '검색에 실패했습니다.');
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.grid}>
        ...
        {
        <div style={styles.card}>
            <h2 style={styles.subHeader}>3.5. 내용으로 검색</h2>
            <form onSubmit={handleContentSerach}>
              <input
                type="text"
